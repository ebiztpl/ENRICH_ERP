    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'writeoff' => $this->input->post('writeoff')??0,
                ...
                if ($ext == 'csv') {
                    ...
                    if (!empty($result)) {
                        ...
                        foreach ($result[$i] as $r_key => $r_value) 
                        {
                           ...
                           if( $fields[$n] == 'publish'){
                            $name = trim($this->encoding_lib->toUTF8($result[$i]['publish']));
                            //  echo $name;
                            $this->db->select('*')->from('library_dropdown_data');
                            ...
                            $check2 = $check2->row();
                            if($check2){
                                $book_data[$i][$fields[$n]]  = $check2->id;
                        ...
                        if( $fields[$n] == 'author'){
                            $name = trim($this->encoding_lib->toUTF8($result[$i]['author']));
                            //  echo $name;
                            $this->db->select('*')->from('library_dropdown_data');
                        ...
                        if( $fields[$n] == 'subject'){
                            $name = trim($this->encoding_lib->toUTF8($result[$i]['subject']));
                            //  echo $name;
                            $this->db->select('*')->from('library_dropdown_data');
                        ...
                        if( $fields[$n] == 'book_language'){
                            $name = trim($this->encoding_lib->toUTF8($result[$i]['book_language']));
                            //  echo $name;
                            $this->db->select('*')->from('library_dropdown_data');
                        ...
                        if( $fields[$n] == 'department'){
                            $name = trim($this->encoding_lib->toUTF8($result[$i]['department']));
                            //  echo $name;
                            $this->db->select('*')->from('library_dropdown_data');
                            $this->db->where('type',5);
                            $this->db->like('name',$name, 'none');
...
$this->session->set_userdata(array(
    ...
    'dep'   =>  $dep,
        ...
        $listbook         = $this->book_model->getbooklist2($id,$book_title,$bookcategory,$author,$publisher,$avaiblity,$created_by,$created_from,$created_to,$writeoff,$pub,$auth,$sub,$dep);
        $data['listbook']  = $listbook;
...
$writeoff = $this->session->userdata('writeoff');
// echo $book_title; echo "<br>";
$pub = $this->session->userdata('pub');
