...
class Student extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        $state                = $this->class_model->getstate();
 ...
 // end here for valiation of new fields
        ...
        if ($this->form_validation->run() == false) {
            ...
            //checking enrollment number is unique or not
            // $enrollment_no_exists = $this->student_model->check_enrollment_no_exists($enrollment_no);
            // if ($enrollment_no_exists) {
            // if (isset($enrollment_no) && !empty($enrollment_no)) {
            //     $insert = false;
            // } else {
            // if (isset($enrollment_no)) {
            //     $data_insert['enrollment_no'] = trim($this->input->post('enrollment_no'));
            // }
            // }
            ...
            if ($insert) {
                ...
                if (!empty($subarray)) {
                    ...
                    $student_session_sub = $this->studenttransportfee_model->addd($subarray_data_insert, $student_session_id);
                } 
                ...
                {
                    // echo "<pre>";
                    // print_r($first_titlee);
                    foreach($first_titlee as $key => $ptitle)
                    {
                        if (isset($_FILES["first_doc"]) && !empty($_FILES['first_doc']['name'][$key])) {
                            $imp         = $this->media_storage->ritkfileupload("first_doc", $upload_directory,$key);
                            $data_img    = array('student_id' => $insert_id, 'title' => $ptitle, 'doc' => $imp);
    ...
    public function import()
    {
        ...
        $fields = array('admission_no', 'roll_no', 'firstname', 'middlename', 'lastname', 'gender', 'dob', 'category_id', 'religion', 'cast', 'mobileno', 'email','state','city','admission_date', 'blood_group', 'school_house_id', 'height', 'weight', 'measurement_date', 'father_name', 'father_phone', 'father_occupation', 'mother_name', 'mother_phone', 'mother_occupation', 'guardian_is', 'guardian_name', 'guardian_relation', 'guardian_email', 'guardian_phone', 'guardian_occupation', 'guardian_address', 'current_address', 'permanent_address', 'bank_account_no', 'bank_name', 'ifsc_code', 'adhar_no', 'samagra_id', 'rte', 'previous_school', 'note','application_no');
                ...
                if ($ext == 'csv') {
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            $n                = 0;
                        //  echo    $result[2]['admission_no'];die;
                         if($result[$i]['admission_no'] != ''){
                            ...
                            if ($this->sch_setting_detail->adm_auto_insert) {
                                ...
                                if ($admission_no_exists) {
                                    ...
                                    $this->db->like('name', $result[$i]['state']??'Madhya Pradesh');
                                    $stat = $this->db->get()->row_array();
                                    $student_data[$i]['state'] = $stat['id'];
         
                                     $this->db->select('id');
                                     $this->db->from('ci_cities');
                                     $this->db->like('name', $result[$i]['city']??'gwalior');
                                     $city = $this->db->get()->row_array();
                                     
                                     $student_data[$i]['city'] = $city['id'];
    ...
    public function edit($id)
    {
       ...
       if ($this->sch_setting_detail->subject_option_req) {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $family_mid_no = $this->input->post('family_mid_no');
            if (isset($family_mid_no)) {
                $data['family_mid_no'] = $family_mid_no;
            }

            $apar_id = $this->input->post('apar_id');
            if (isset($apar_id)) {
                $data['apar_id'] = $apar_id;
            }

            $school_medium = $this->input->post('school_medium');
            if (isset($school_medium)) {
                $data['school_medium'] = $school_medium;
            }
            ...
            $scholarship_form_no = $this->input->post('scholarship_form_no');
            if (isset($scholarship_form_no)) {
                $data['scholarship_form_no'] = $scholarship_form_no;
            }
            ...
            $class_school = $this->input->post('class');
            if (isset($class_school)) {
                $data['class'] = $class_school;
            }
          ...
          // new field end
            ...
            if (!empty($subarray)) {
                ...
                $student_session_sub = $this->studenttransportfee_model->updatee($subarray_data_insert, $student_session_id);
            } else {
                $student_session_sub = $this->studenttransportfee_model->updatee([], $student_session_id);
            }
            ...
            {
                 $returnids = array();
                $check_first_title = $this->input->post('check_first_title');

                foreach($first_titlee as $key => $ptitle)
                {   

                    if($check_first_title[$key] == 0){
                        if (isset($_FILES["first_doc"]) && !empty($_FILES['first_doc']['name'][$key])) {
                            $imp         = $this->media_storage->ritkfileupload("first_doc", $upload_directory,$key);
                            $data_img    = array('student_id' => $id, 'title' => $ptitle, 'doc' => $imp);
                         $returnids[] =   $this->student_model->adddoc($data_img);
                        }
                    }

                }
                $this->db->select('id')->from('student_doc');
                $this->db->where('student_id',$id);
                $this->db->order_by('student_doc.id', 'desc');
                $check2 = $this->db->get();
                ...
                $to_bedeleted=array_diff($arr2,$previous_id);
                // print_r($returnids);
                // print_r($to_bedeleted);
                // print_r($returnids);
...
//  print_r($to_bedeletedd);die;
    ...
    public function dtstudentlist()
    {
        ...
        if ($search_type == "search_filter") {
           ...
           $gender      = $this->input->post('gender');
               ...
               $checkkdel1  = $this->input->post('checkkdel1');
               $checkkdel2 = $this->input->post('checkkdel2');
               $checkkdel3  = $this->input->post('checkkdel3');
        ...
        if (!empty($students->data)) {
            foreach ($students->data as $student_key => $student) {
                ...
                $viewbtn = "<a href='" . base_url() . "student/journey/" . $student->id . "'   class='btn btn-default btn-xs'  data-toggle='tooltip' title='Student Journey'><i class='fa fa-mail-reply'></i></a><a href='" . base_url() . "student/view/" . $student->id . "'   class='btn btn-default btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('view') . "'><i class='fa fa-reorder'></i></a>";
  ...
  $row[] = $student->application_no;
                ...
                $row[] = date('d-m-Y',strtotime($student->created_at));

                $row[] = '<span>'.$viewbtn . '' . $editbtn . '' . $collectbtn.'</span>';
    ...
    {    
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'drop_note'   => $this->input->post('note'),
                'id'         => $this->input->post('student_id'),
                'dropout_at' => $this->customlib->dateFormatToYYYYMMDD($this->input->post('disable_date')),
                'is_dropout'  => 1,
            );
    ...
    public function disabledropout($id)
    {
        $data = array('is_dropout' => 0);
        $this->student_model->disableStudent($id, $data);
        echo "0";
    }


    public function dropoutstudentslist()
    {
        // echo 1;die;
        if (!$this->rbac->hasPrivilege('disable_student', 'can_view')) {
            access_denied();
        ...
        // $this->session->set_userdata('sub_menu', 'student/disablestudentslist');
        $this->session->set_userdata('top_menu', 'Student Information');
        $this->session->set_userdata('sub_menu', 'student/dropoutstudentslist');
        $class                   = $this->class_model->get();
        $data['classlist']       = $class;
        $data["resultlist"]      = array();
        $data['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $data['search_text'] = $this->input->post('search_text');
                        $resultlist          = $this->student_model->dropoutstudentByClassSection($class, $section);
                    ...
                    $data['search_text'] = trim($this->input->post('search_text'));
                    $resultlist          = $this->student_model->dropoutstudentFullText($search_text);
                    $data['resultlist']  = $resultlist;
                }
            }
        }   
        ...
        $this->load->view("layout/header", $data);
        $this->load->view("student/dropoutstudents", $data);
        $this->load->view("layout/footer", $data);
    }
    ...
    public function journey($id)
    {
        ...
        $studentsessions           = $this->student_model->getstudentsessions($id);
      

         $student_due_fee = array();
         
           foreach($studentsessions as $studentt){
            $student_due_fee[]              = $this->studentfeemaster_model->getStudentFeesjourney($studentt['id'],$studentt['session_id']);
...
// print_r($student_due_fee);die;
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('student/studentJourney', $data);
        $this->load->view('layout/footer', $data);
    }
