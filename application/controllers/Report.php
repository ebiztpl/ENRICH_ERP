...
public function totalAccessionReport()
    ...
    public function getbookissueparameter()
    {
        ...
        $params = array('book_category' => $book_category,'search_type' => $search_type, 'members_type' => $members_type, 'date_from' => $date_from, 'date_to' => $date_to);
        $array  = array('status' => 1, 'error' => '', 'params' => $params);
        echo json_encode($array);
    }
...
public function getAccessionReportparameter()
{
    ...
    public function dtbookissuereportlist()
    {
        ...
        $result = $this->bookissue_model->studentBookIssue_report($start_date, $end_date,$book_category);
        $sch_setting = $this->sch_setting_detail;
        $resultlist = json_decode($result);
        $dt_data    = array();
      ...
      $c =  intval($resultlist->recordsFiltered);
        ...
        //  print_r($resultlist->data);die;
            ...
            foreach ($resultlist->data as $resultlist_key => $value) {
                ...
                //    if(($book_category  == $value->book_category) || ($book_category  == '')){
    ...
    if($book_categoryy){
        ...
        $pq    = 'NA';
                ...
                if ($value->member_type == 'student') {
                    $row[] = '<a href="'.base_url().'/student/view/'.$value->sid.'">'.$this->customlib->getFullName($value->firstname, $value->middlename, $value->lastname, $sch_setting->middlename, $sch_setting->lastname) . $admission.'</a>';
                }elseif($value->member_type == 'guest'){

                    $row[] = '<a href="'.base_url().'/admin/member/guestedit/'.$value->guest_id.'">'.$value->guest_name.'</a>';
                } else {
                    ...
                    if ($superadmin_visible == 'disabled') {
                        if ($staffrole->id != 7) {
                            ...
                            if (isset($staffresult['role_id']) && $staffresult['role_id'] == 7) {
                                ...
                                $row[] = '<a href="'.base_url().'/admin/staff/profile/'.$value->staff_id.'">'.ucwords($value->staff_name) . $staff_employee_id.'</a>';
                            }
                        } else {
                            $row[] = '<a href="'.base_url().'/admin/staff/profile/'.$value->staff_id.'">'.ucwords($value->staff_name) . $staff_employee_id.'</a>';
                        }
    ...
    if($book_categoryy){
        ...
        $pq    = 'NA';
                ...
                $your_date = strtotime(date('Y-m-d', $this->customlib->dateyyyymmddTodateformat($value->duereturn_date)));
                ...
                if($value->member_type == 'student') {
                    ...
                    if($value->is_returned == 1){
                        $row[] = $value->late_fine??0 ;
                    }elseif($value->is_returned == 2){
                        $row[] = $value->late_fine ;
                ...
                }elseif(($this->lang->line($value->member_type) == 'Teacher')){
              ...
              if($value->is_returned == 1){
                ...
                // if ($value->member_type == "student") {
                //     $name  = $this->customlib->getFullName($value->firstname,$value->middlename, $value->lastname, $sch_setting->middlename, $sch_setting->lastname) ;
                ...
                // if($value->member_type == 'student'){
                //     $row[] =   '<a href="'.base_url().'/student/view/'.$value->sid.'">'.$name.'</a>';
                //    }else{

                //     $row[] =     '<a href="'.base_url().'/admin/staff/profile/'.$value->staff_id.'">'.$name.'</a>'; 
                ...
                if ($value->member_type == 'student') {
                    $row[] = '<a href="'.base_url().'/student/view/'.$value->sid.'">'.$this->customlib->getFullName($value->firstname, $value->middlename, $value->lastname, $sch_setting->middlename, $sch_setting->lastname) . ' (' . $value->admission . ')'.'</a>';
                }elseif($value->member_type == 'guest'){
                        ...
                        if ($superadmin_visible == 'disabled') {
                            if ($staffrole->id != 7) {
                                ...
                                if ($staffresult['role_id'] == 7) {
                                    ...
                                    $row[] = '<a href="'.base_url().'/admin/staff/profile/'.$value->staff_id.'">'.ucwords($value->fname) . " " . ucwords($value->lname) . ' (' . $value->employee_id . ')'.'</a>';
                                }
                            } else {
                                $row[] = '<a href="'.base_url().'/admin/staff/profile/'.$value->staff_id.'">'.ucwords($value->fname) . " " . ucwords($value->lname) . ' (' . $value->employee_id . ')'.'</a>';
                            }
    ...
    //             $now = time(); // or your date as well
    //             $your_date = strtotime(date($this->customlib->getSchoolDateFormat(), $this->customlib->dateyyyymmddTodateformat($value->duereturn_date)));
    //             $datediff = $now - $your_date;
...
$val1 = $valcount -$val;
    ...
    public function surrend_returnreport()
    {
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/library');
        $this->session->set_userdata('subsub_menu', 'Reports/library/surrend_returnreport');

        
      
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            $memberList         = $this->librarymember_model->getsurrender();
        } else {
            $class       = $this->input->post('class_id');
            $section     = $this->input->post('section_id');
            $members_type      = $this->input->post('members_type');
            if (isset($members_type)) {
                 ...
                 if($members_type == 'teacher'){
                    $memberList         = $this->librarymember_model->getsurrenderteacher();

                 }elseif($members_type == 'student'){
                    $memberList         = $this->librarymember_model->getsurrenderstudent($class,$section);

                 }elseif($members_type == 'guest'){
                    $memberList         = $this->librarymember_model->getsurrenderguset();
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('reports/surrend_returnreport', $data);
        $this->load->view('layout/footer', $data);
    }
