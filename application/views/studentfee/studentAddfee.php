                ...
                " alt="No Image">
                        ...
                        <div class="row no-print mb10">
                            <div class="col-md-12 mDMb10">
                                ...
                                <button type="button" class="btn btn-sm btn-warning singlereceipt" id="load" data-loading-text="<i class='fa fa-spinner fa-spin '></i> <?php echo $this->lang->line('please_wait') ?>"><i class="fa fa-print"></i> Single Fee Receipt</button>

                                <?php if ($this->rbac->hasPrivilege('collect_fees', 'can_add')) { ?>
                                <button type="button" style="display:none" class="btn btn-sm btn-warning collectSelected" id="load" data-loading-text="<i class='fa fa-spinner fa-spin '></i> <?php echo $this->lang->line('please_wait') ?>"><i class="fa fa-money"></i> <?php echo $this->lang->line('collect_selected'); ?></button>
                                
                                
                                <button type="button" style="display:none" class="btn btn-sm btn-success collectAutomated" id="load" data-loading-text="<i class='fa fa-spinner fa-spin '></i> <?php echo $this->lang->line('please_wait') ?>"><i class="fa fa-money"></i> Collect Automated</button>
                                
                                <button type="button" class="btn btn-sm btn-dark Collectinorder" style="background-color:#2f81c7a1" id="load" data-loading-text="<i class='fa fa-spinner fa-spin '></i> <?php echo $this->lang->line('please_wait') ?>"><i class="fa fa-money"></i> Collect In Order</button>
                                <?php } ?>
                            ...
                            <table class="table table-striped table-bordered table-hover example table-fixed-header">
                                <thead class="header">
                                    <tr>
                                        ...
                                        <th class="text text-left">Reference No.</th>
...
if ($fee_deposits_value->description == "") {
                ...
                ?>
                                                        ...
                                                        <?php echo $fee_deposits_value->receipt_no??''; ?>
                                                        </td>  
...
$total_bulk_discount = $this->studentfeemaster_model->getStudent_bulkdiscounts_Sum([$fee->student_session_id]);
                                                   ...
                                                   if($total_bulk_discount){
                                                    ?>

                                                            <tr class="dark-light success">
                                                            <td></td>
                                                                <td  align="left"  style="font-size: large;color: black;">Total</td>
                                                                <td  style="font-size: large;color: black;"><?php echo $this->lang->line('discount'); ?></td>
                                                                <td style="font-size: large;color: black;"></td>
                                                                <!-- <td > </td> -->
                                                                
                                                               
                                                                <td class="text-right" style="font-size: large;color: black;">: <?= amountFormat($total_bulk_discount->discount) ?></td>
                                                                <td></td>
                                                                ...
                                                                <td align="left" class="text text-left" >
                                                                        ...
                                                                        <!-- <a href="#" data-toggle="popover" class="detail_popover" style="color:red">


                                                                            <?php echo   $currency_symbol . amountFormat($total_bulk_discount->discount); ?>   
                                                                ...
                                                                <!-- <td style="color:red">Assigned</td>
                                                                <td style="color:red">For </td> -->
                                                                <td style="color:red"></td>
                                                                <td style="color:red"></td>
                                                               
                                                                <td align="left" class="text text-left">
                                                                
                                                                <a class="btn btn-info btn-xs detailed_discount" data-id="<?= $fee->student_session_id ?>" style="display: inline-block;">View</a>
                                                                </td>
...
//  start single receipt


$(document).on('click', '.singlereceipt', function () {
            var array_to_print = [];
                var $this = $(this);
         ...
         //   } else {
                $.ajax({
                    url: '<?php echo site_url("studentfee/printsingleFeesByGroupArray") ?>',
                    type: 'post',
                    data: {'data': JSON.stringify(array_to_print)},
                     beforeSend: function () {
                    $this.button('loading');
...
<script>
$(document).on('click', '.collectAutomated', function () {
            var $this = $(this);
            var array_to_collect_fees = [];
            $.each($("input[name='fee_checkbox']:checked"), function () {
            ...
            $.ajax({
                type: 'POST',
                url: base_url + "studentfee/getautomatedcollectfee",
                data: {'data': JSON.stringify(array_to_collect_fees)},
                dataType: "JSON",
                beforeSend: function () {
                    $this.button('loading');
...
$(document).on('click', '.ritik_automated_button', function () {
            $newamoynt =   $('.ritik_automated').val();
            // alert($newamoynt);

            var $this = $('.collectAutomated');
            var array_to_collect_fees = [];
            $.each($("input[name='fee_checkbox']:checked"), function () {
...
$(document).on('click', '.Collectinorder', function () {
            ...
            var numberOfChecked = $("input[name='fee_checkbox']:checked").length;
           ...
           if(numberOfChecked > 0){
            ...
            alert('Kindly Select atleast one checkbox to continue');
        }
...
$(document).on('click', '.ritik_inorder_button', function () {
        ...
        <script>
            $(document).on('change keyup', '.ritik_inorder', function () {
                $abc = parseInt($(this).val());
                $bcd = parseInt($('.inorder_submit').attr('data-id'));

               if($abc > $bcd){
                
                 $('.amount_error').html('Amount can not be greater then Total Amount "'+ $bcd  + '"');
                //  alert('Amount can not be greater then Total Amount "'+ $bcd  + '"');
                $('.inorder_submit').prop('disabled', true);
                $('.ritik_inorder_button').prop('disabled', true);
               }else{
                $('.amount_error').html('');
                $('.inorder_submit').prop('disabled', false);
                $('.ritik_inorder_button').prop('disabled', false);
