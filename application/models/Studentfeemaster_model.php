...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null)
    {
        $sql = "SELECT IFNULL(`student_fees_master`.`id`, '0') as `student_fees_master_id`,`classes`.`id` AS `class_id`,"
            . " `student_session`.`id` as `student_session_id`, `students`.`id`, `students`.`school_house_id`,"
    ...
    public function delete($fee_session_groups, $array)
    {
        ...
        $query = $this->db->get();
        $feemaster_id_aray = $query->result();




        if ($query->num_rows() > 0) {
            foreach ($feemaster_id_aray as $feemaster_id) {

                $this->db->where('fee_master_id', $feemaster_id->id);
    ...
    public function getStudentFees($student_session_id)
    {
        ...
        -- INNER JOIN fee_groups_feetype on fee_session_groups.id  =  fee_groups_feetype.fee_session_group_id
        INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id 
        --   INNER JOIN student_fees_master_head on student_fees_master.id = student_fees_master_head.fee_master_id AND student_fees_master_head.head_id=fee_groups_feetype.id
    ...
    public function getDueFeeByFeeSessionGroup($fee_session_groups_id, $student_fees_master_id)
    {
        ...
        INNER JOIN student_fees_master_head on  student_fees_master_head.fee_master_id  = student_fees_master.id AND student_fees_master_head.head_id=fee_groups_feetype.id
    ...
    {
        $sql = "SELECT student_fees_master.id,student_fees_master.is_system,feetype.is_donation as donation ,student_fees_master.student_session_id,student_fees_master.fee_session_group_id,student_fees_master.amount as
            ...
            foreach ($data as $d_key => $d_value) {
                ...
                if ($q->num_rows() > 0) {
                    ...
                    $collected_fees[] = array(
                        ...
                        'student_transport_fee_id' => $data[$d_key]['student_transport_fee_id'],
                        'fee_category' => $d_value['fee_category'],
                        'amount_s' => json_decode($data[$d_key]['amount_detail'], true)[1]['amount']
                    );
                }
            }
    ...
    public function addd($data,$category_head_array)
    {
        $this->db->where('student_session_id', $data['student_session_id']);
        $this->db->where('fee_session_group_id', $data['fee_session_group_id']);
        $q = $this->db->get('student_fees_master');
        ...
        if ($q->num_rows() > 0) {
             $inserted_id  =   $q->row()->id;
            $insert_arrayy = array();

            $this->db->select('head_id')->from('student_fees_master_head');
            $this->db->where('fee_master_id`',$inserted_id);
            $this->db->order_by('student_fees_master_head.id', 'desc');
            $check2 = $this->db->get();
            $check2 = $check2->result_array();
            $arr2 = array_map (function($value){
                return $value['head_id'];
            } , $check2);

           foreach ($category_head_array as $key => $value) {
                   if(!in_array($value, $arr2)){
                   $insert_arrayy[] = array(
                       'head_id'   => $value,
                       'fee_master_id' => $inserted_id,
                   );
                }

           }
           $this->db->insert_batch('student_fees_master_head', $insert_arrayy);
        } else {
            $this->db->insert('student_fees_master', $data);
            $id = $this->db->insert_id();
            $message   = INSERT_RECORD_CONSTANT . " On student fees master id " . $id;
    ...
    public function getStudentFees2($student_session_id)
    {
        ...
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
                ...
                $fee_session_group_id   = $result_value->fee_session_group_id;
                $student_fees_master_id = $result_value->id;
                $result_value->fees     = $this->getDueFeeByFeeSessionGroup2($fee_session_group_id, $student_fees_master_id);
    ...
    public function getDueFeeByFeeSessionGroup2($fee_session_groups_id, $student_fees_master_id)
    {
        ...
        $sql = "SELECT student_fees_master.*,fee_groups_feetype.id as `fee_groups_feetype_id`,fee_groups_feetype.amount,fee_groups_feetype.due_date,fee_groups_feetype.fine_amount,fee_groups_feetype.fee_groups_id,fee_groups.name,fee_groups_feetype.feetype_id,feetype.code,feetype.type, IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` FROM `student_fees_master`
         ...
         INNER JOIN fee_groups on fee_groups.id=fee_groups_feetype.fee_groups_id
          INNER JOIN feetype on feetype.id=fee_groups_feetype.feetype_id 
          LEFT JOIN student_fees_deposite on student_fees_deposite.student_fees_master_id=student_fees_master.id and student_fees_deposite.fee_groups_feetype_id=fee_groups_feetype.id WHERE student_fees_master.fee_session_group_id =" . $fee_session_groups_id . " and student_fees_master.id=" . $student_fees_master_id . " order by fee_groups_feetype.id ASC";
    ...
    public function getFeeByrecipt($reciptnumber)
    {
        $type = $this->db->select('`student_fees_deposite`.*')->from('`student_fees_deposite`')->like('amount_detail', $reciptnumber)->get()->row_array();
        if (empty($type['student_transport_fee_id'])) {
            $this->db->select('`student_fees_deposite`.*,students.id as std_id,students.firstname,students.middlename,students.lastname,students.admission_no,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,`fee_groups`.`name`, `feetype`.`type`, `feetype`.`code`,feetype.is_system,student_fees_master.student_session_id,student_session.session_id')->from('student_fees_deposite');
            $this->db->join('fee_groups_feetype', 'fee_groups_feetype.id = student_fees_deposite.fee_groups_feetype_id');
            $this->db->join('fee_groups', 'fee_groups.id = fee_groups_feetype.fee_groups_id');
            ...
            $this->db->join('students', 'students.id=student_session.student_id');
            // $this->db->where('student_fees_deposite.id', $invoice_id);
            $this->db->like('student_fees_deposite.amount_detail', $reciptnumber);
            $q = $this->db->get();
            if ($q->num_rows() > 0) {
                $result = $q->result();
                // $res    = json_decode($result->amount_detail);
    ...
    public function getStudentFeesjourney($student_session_id,$session)
    {
        $sql    = "SELECT `student_fees_master`.*,fee_groups.name FROM `student_fees_master`
         INNER JOIN fee_session_groups on student_fees_master.fee_session_group_id=fee_session_groups.id 
         INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id 
...
// print_r($result);die;
    ...
    public function getStudentFees3($student_session_id)
    { 
        $sql    = "SELECT `student_fees_master`.*,fee_groups.name FROM `student_fees_master` 
        INNER JOIN fee_session_groups on student_fees_master.fee_session_group_id=fee_session_groups.id 
        INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id 
         WHERE `student_session_id` IN  (". implode(',',$student_session_id)  . ") ORDER BY `student_fees_master`.`id`";
        $query  = $this->db->query($sql);
        $result = $query->result();
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
...
public function getStudent_bulkdiscounts_Sum($student_session_id)
{
    ...
    { 
        ...
        $sql    = " SELECT * FROM `student_bulk_discount` 
