...
if($branch_id == 1){
    ...
    $branch_query = Branch::query();
	$branch_query->where('id',$branch_id);
}
    ...
    public function ledger_income_expense_head_wise_report(Request $request)
    {
        ...
        $income_expense_head_query->with(['Transaction' => function ($query) use ($request,$branch_id) {
...
$query->where('branch_id', $branch_id);
    ...
    public function ledger_bank_cash_wise_report(Request $request)
    {
        ...
        if($branch_id != 1){
            ...
            $search_by = [
                'branch_name' => ($branch_id != null) ? $branches->where('id', $branch_id)->first()->name : '',
   ...
   if($branch_id != 1){
        $bank_caches_balance = Helper::__bank_cash_details($request->bank_cash_id, $branch_id, $request->from, $request->to);
        }else{
